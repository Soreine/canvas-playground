...
function draw(ctx, frame) {
  ...
  const prog = progress(frame);

  const progs = createArray(N_HEXAGON, function (i) {
    return nProgress(prog, i, N_HEXAGON);
  });
...
function hexagon(ctx, radius, rotation, holePercent) {
  const shape = polygon(SIDES, radius, new Coord(CENTER, CENTER), rotation);
  const segments = getSegments(shape);
  const withHole = segments
    .map(function (segment) {
      return holeSegment(segment[0], segment[1], holePercent);
    })
    .reduce(
      // Reduce to a list of segments
...
function grayPercent(percent, alpha) {
  const v = Math.floor(255 * percent);
  alpha = alpha || 100;
  return `rgba(${v},${v},${v},${alpha})`;
}
...
function getSegments(vertices) {
  ...
  res.push([vertices[vertices.length - 1], vertices[0]]);
